...
class Integrasi extends CI_Controller {
	...
	public function monitoring (){
		...
		if (!empty($cfg['icon'])) $this->db->where('t_logger.icon',$cfg['icon']);
		if ($id_kategori!=='1' && $id_kategori!=='2') $this->db->where('katlog_id',$id_kategori);
	...
	public function horizontal (){
		...
		if (!$data_logger) { echo json_encode([]); return; }
		...
		foreach($loggers as $code){
			if(!isset($sensor_map[$code])) continue;
			$col = $sensor_map[$code]['field'];
			$cases[]="WHEN code_logger='{$code}' THEN {$col}";
		}
		if(!$cases){ echo json_encode([]); return; }
    ...
    ",false)
		...
		if($id_kategori == 'arr' or $id_kategori == 'awr'){
			$warnaJam=[
				[0,0,'#D5F0C1'],[0.1,1,'#70cddd'],[1,5,'#35549d'],
				[5,10,'#fef216'],[10,20,'#f47e2c'],[20,INF,'#ed1c24']
			];
			$warnaTotal=[
				[0,0,'#98bc85'],[0.1,5,'#70cddd'],[5,20,'#35549d'],
				[20,50,'#fef216'],[50,100,'#f47e2c'],[100,INF,'#ed1c24']
			];
			foreach($data_rekap as &$dtr){
				foreach($dtr['data'] as &$row){
					...
					foreach($warnaJam as $r){
						if($v>=$r[0] && $v<$r[1]){ $row['warna']=$r[2]; break; }
					}
				}
				$dtr['warna']='#98bc85';
				$vt=(float)$dtr['total'];
				foreach($warnaTotal as $r){
					if($vt>=$r[0] && $vt<$r[1]){ $dtr['warna']=$r[2]; break; }
				}
			}
			unset($dtr,$row);
		}
			...
			foreach ($data['data_rekap'] as $key=> $dtr) {
				...
				foreach ($dtr['data'] as $k2=> $dt_q) {
					if ($dt_q['nilai'] !== '-') {
						$v = (float)$dt_q['nilai'];
						$total_hujan += $v;
						if ($v <= 0.0) $data['data_rekap'][$key]['data'][$k2]['warna'] = '#D5F0C1';
						elseif ($v >= 0.1 && $v < 1)  $data['data_rekap'][$key]['data'][$k2]['warna'] = '#70cddd';
						elseif ($v >= 1   && $v < 5)  $data['data_rekap'][$key]['data'][$k2]['warna'] = '#35549d';
						elseif ($v >= 5   && $v < 10) $data['data_rekap'][$key]['data'][$k2]['warna'] = '#fef216';
						elseif ($v >= 10  && $v < 20) $data['data_rekap'][$key]['data'][$k2]['warna'] = '#f47e2c';
						elseif ($v >= 20)             $data['data_rekap'][$key]['data'][$k2]['warna'] = '#ed1c24';
						else                           $data['data_rekap'][$key]['data'][$k2]['warna'] = '#D5F0C1';
					} else {
						$data['data_rekap'][$key]['data'][$k2]['warna'] = '#D5F0C1';
					}
				}
				if     ($total_hujan <= 0)                    $wrn = '#98bc85';
				elseif ($total_hujan >= 0.1 && $total_hujan < 5)   $wrn = '#70cddd';
				elseif ($total_hujan >= 5   && $total_hujan < 20)  $wrn = '#35549d';
				elseif ($total_hujan >= 20  && $total_hujan < 50)  $wrn = '#fef216';
				elseif ($total_hujan >= 50  && $total_hujan < 100) $wrn = '#f47e2c';
				elseif ($total_hujan >= 100)                        $wrn = '#ed1c24';
	...
	public function pilihpos()
	{
		...
		foreach ($q_pos->result() as $pos) {
			$data[] = array(
				'idLogger' => $pos->code_logger.'_psda', 'namaPos' => $pos->nama_lokasi
			);
		}
